...
- 20 MCP tools: read/write/search/list/recent/graph/orphans/stats, batch tag &
  ...
  auto-generated MOC index (记忆索引.md), routing index (路由索引.md)
...
- Vault layout: notes may live in domain subfolders (规则/项目/知识/人设与偏好/日志);
  the server scans recursively and excludes system dirs (.archive/.trash/.workbuddy)
"""
...
# vault, e.g.  AI_MEMORY_DIR=~/my-vault python server.py
...
SYSTEM_DIR_NAMES = {".archive", ".trash", ".workbuddy", ".git", ".obsidian", "__pycache__", ".trash_backup"}
...
def _folder_for_title(title: str) -> str | None:
    ...
    if title.startswith("知识_"):
        return "知识"
    if title.startswith("日志_") or title.startswith("工作动态_") or re.match(r"^\d{4}-\d{2}-\d{2}", title):
        return "日志"
...
def _refresh_routing_index() -> None:
    """Rebuild 路由索引.md with one compact line per note (title|folder|date|tags|summary).
    ...
    try:
        ...
        lines = [
            "# 路由索引（自动维护）",
            "",
            "> 两级检索：先在此文件定位（每条 = 标题 | 目录 | 日期 | 标签 | 一句话摘要），",
            "> 再按需 [[记忆索引]]/memory_read 读取叶子笔记。请勿手工编辑本文件。",
            "",
        ]
        index_meta_links: list[str] = []
        for path, meta, _body in entries:
            ...
            tags = ", ".join(_entry_tags(meta)) or "-"
            summary = str(meta.get("summary", "")).strip().replace("\n", " ")
            lines.append(f"- [[{title}]] | {folder} | {created} | {tags} | {summary}")
            if title == "记忆索引":
                index_meta_links.append(ROUTING_INDEX_TITLE)
            if title in {"记忆索引", "近期工作动态", "项目状态", "用户画像", "AI人格"}:
                index_meta_links.append(title)
        lines.append("")
        lines.append("> 目录路由：规则_* → 规则/；项目_* → 项目/；知识_* → 知识/；AI人格/用户画像 → 人设与偏好/；")
        lines.append("> 日志_*/工作动态_* 或日期开头 → 日志/；其余新笔记默认库根，可在规则中指定目录。")
        body = "\n".join(lines).rstrip()
...
CORE_PAGES = {"记忆索引", "路由索引", "近期工作动态", "项目状态",
              "用户画像", "AI人格", "规则_记忆写入格式规范",
              "规则_Git版本控制规范"}
...
def _refresh_index() -> None:
    ...
    try:
        ...
        if isinstance(existing_links, list):
            merged_links = existing_links + [ROUTING_INDEX_TITLE]
            meta["links"] = list(dict.fromkeys(merged_links))
        else:
            meta["links"] = [ROUTING_INDEX_TITLE]
        meta["updated"] = datetime.now(timezone.utc).isoformat()
        _atomic_write_text(idx_path, f"---\n{_dump_yaml_frontmatter(meta)}\n---\n\n{body}")
    except Exception as e:
